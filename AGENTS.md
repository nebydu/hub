...
- 변수명과 함수명은 영어로 작성한다(Java 표준 컨벤션 준수).
...
- 변경 후 가능한 경우 테스트와 lint를 실행한다.
  - Java/Maven: `./mvnw test`, `./mvnw verify`
  - 빌드 도구는 Gradle이 아닌 **Maven**을 사용한다(워크스페이스 정책).
...
## 프로젝트 컨텍스트 (hub)
- 언어/런타임: Java + Spring Boot
- 빌드 도구: Maven (워크스페이스 정책상 Gradle override)
- 역할: monitoring 데모의 Hub — Kafka 컨슈머/프로듀서, Thymeleaf UI, 스케줄러
- 인접 모듈: `../script-agent` (Go), `../infra` (docker-compose: Kafka 9092 / OTel 14318)

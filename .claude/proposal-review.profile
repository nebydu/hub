# proposal-review.profile — hub consumer 델타 (H6)
# /proposal-review command가 Codex 리뷰에 주입할 hub 문맥. 도메인 결정은 이 파일에만 둔다.
...
_HUB_ROOT="$(git rev-parse --show-toplevel)"
...
PROPOSAL_REVIEW_CONTEXT_DOCS=(
  "$_HUB_ROOT/README.md"
  "$_HUB_ROOT/AGENTS.md"
  "$_HUB_ROOT/../monitoring-meta/docs/phase0-snapshot/monitoring-demo-message-spec-v0.2.1.md"
  "$_HUB_ROOT/../monitoring-meta/docs/kafka-payloads.md"
  "$_HUB_ROOT/../monitoring-meta/docs/envelope.md"
)

PROPOSAL_REVIEW_POLICY="hub는 Java/Spring 런타임 코드 repo다. 결정 리뷰 시 지킬 기준: 방향 판단의 최상위 기준은 통합본 v0.9(이 입력에 미포함, 170KB라 제외 — ../monitoring-meta/docs/통합본_v0_9.md)이고, 데모 spec v0.2.1은 Phase 0 회귀 방지 가드다. 제안이 'Phase 0 유지'인지 'Phase 1+ 선반영'인지 분류하지 않았으면 결함으로 지적하라. 통합본의 Phase 1+ 목표를 Phase 0 코드에 무조건 강제하는 제안, [Open]/미결정 ADR을 결정된 것으로 전제한 제안, 형제 repo(script-agent/monitoring-meta/infra)를 hub가 직접 수정하는 것을 전제한 제안은 block 대상이다. 코드 작업 spec은 ../monitoring-meta/handoff/<work-id>-hub.md 경유가 원칙이므로 이를 우회하는 프로세스 제안도 결함으로 지적하라. 통합본이 직접 쟁점인데 발췌가 없으면 missing_context로 지적하라."

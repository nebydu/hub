...
## 출력 — 분석 본문 + 마지막 결과 스키마
구조화된 markdown으로 ① 현황(Phase 0 동작) ② 통합본 기준 요구 ③ 작업 단계 분류(Phase 0 유지 vs Phase 1+ 선반영) ④ Phase 0 회귀 가드(데모 spec) ⑤ 구현 단계 분해 ⑥ 영향 범위(어느 패키지/파일) ⑦ **미결정 사안(사람 입력 대기)**을 정리한 뒤, 마지막에 아래 JSON을 출력한다:
```json
{
